              ...
              <v-col align="center" cols="12" lg="3" md="3" xs="12">
                <template
                  v-if="
                    ...
                    attachment.name.split('.').pop().toLowerCase() === 'jpeg' ||
                    attachment.name.split('.').pop().toLowerCase() === 'png' ||
                    attachment.name.split('.').pop().toLowerCase() === 'tif' ||
                    attachment.name.split('.').pop().toLowerCase() === 'bmp'
                  "
                >
                  <v-card>
                    <v-img
                ...
                <template v-else>
                  <v-icon
                    v-if="
                      attachment.name.split('.').pop().toLowerCase() === 'pdf'
                    "
                    size="60"
                    file-word-outline
                    color="red darken-1"
...
export default {
  ...
  methods: {
    ...
    async uploadFieldChange() {
      this.btnLoader = true
      for (const [index, item] of this.tempAttachment.entries()) {
        if (this.documentAttachment.length < this.maxFileCount) {
          if ((item.size / 1000).toFixed(1) > this.maxFileSize) {
            ...
            const fullFileType = this.readerFile.split(';')
            const fileType = fullFileType[0].split(':')
            let status = false
            let imgFile = ''
            let sizeInKb = 0
            ...
            const strTemp = this.readerFile.split(',')
            if (status) {
              const imgTemp = imgFile.split(',')
              tempFile.base64 = imgTemp[1]
              sizeInKb = Buffer.from(imgFile, 'base64').length
    ...
    compressImage(base64) {
      ...
      return new Promise((resolve, reject) => {
        const imageCompressLevelTemp = this.imageCompressLevel
        img.onload = function () {
    ...
    UploadFile(file) {
      ...
      // sending file to backend Mudar isso depois em
      this.$axios
        .post('/produto/upload', formData, {
